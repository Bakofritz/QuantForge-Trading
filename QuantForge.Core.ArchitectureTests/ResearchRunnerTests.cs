...
public class ResearchRunnerTests
{
    ...
    [Fact]
    public void Runner_rejects_ledger_strategy_mismatch_without_performance_state()
    {
        var request = CreateRequest(
            new[]
            {
        ...
        Assert.Null(report.Account);
        Assert.Null(report.EvidenceTail);
        Assert.Contains("ledger namespace", report.BlockReason);
    }

    [Fact]
    public void Runner_converts_execution_domain_failure_to_invalid_report()
    {
        var request = CreateRequest(
            new[]
            {
