...
### v30.06a — Diagnostic acknowledgment/outcome review correction
Before device delivery, source review found that Mark problem said recorded even if its bounded queue refused an event. It now reports queued locally only on acceptance and explicitly reports a refused marker otherwise. Import logging now distinguishes unavailable/cancelled/invalid results rather than collapsing every unsuccessful result to invalid. Baseline b3deeecfcae94080d2dc2fdda249155d833af843 core tests passed; this review correction is not a failed compiler/test revision. Revalidate the new exact head and signed APK. User-facing test version remains 0.30.6/code 3006; source SHA in the assembly/artifact disambiguates the undelivered candidates. No previous APK has been delivered. No scope/authority change.


### v30.06b — Trim-safe diagnostic JSON and absolute event ceiling
Native release review identified reliance on reflection-based JSON serialization, which can be disabled in trimmed applications. Replaced diagnostic serialization/deserialization with compile-time System.Text.Json metadata for the fixed environment/event/status/manifest types. No raw-object serializer or widening of recorded data is added. Strengthened the absolute event ceiling to apply even to repeated stop-action inputs. This is a pre-delivery review correction; no APK was delivered from earlier candidates. Exact-head tests/native/signature checks must pass. Microsoft reference: https://learn.microsoft.com/dotnet/standard/serialization/system-text-json/source-generation . User-facing version remains 0.30.6/code 3006; source SHA disambiguates candidates. Live authority and APK-test approval boundaries unchanged.


### v30.06c — Reflection-disabled manifest/diagnostic validation
Extended generated JSON metadata to the existing ResearchManifest codec, preserving camelCase names, numeric authority representation and strict prevalidation. Added a dedicated CI run of diagnostic and manifest-reader regressions with JsonSerializerIsReflectionEnabledByDefault=false, to exercise the native release constraint rather than assuming desktop test behavior proves Android compatibility. This final pre-delivery candidate remains app version 0.30.6/code 3006 and must pass both native gates/signature verification. Earlier undelivered candidates are superseded, not labelled failed absent actual failed checks.
